...
func NewAdminHandler(svc *service.AdminService, templatesDir string, jwtSecret []byte, jwtTTL time.Duration) *AdminHandler {
	return &AdminHandler{svc: svc, templatesDir: templatesDir, jwtSecret: jwtSecret, jwtTTL: jwtTTL}
}
...
func (h *AdminHandler) GetRecord(w http.ResponseWriter, r *http.Request) {
	data, err := h.svc.GetRecord(r.URL.Query().Get("table"), r.URL.Query().Get("id"))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *AdminHandler) collectFields(r *http.Request) map[string]string {
	return map[string]string{
		"login": r.FormValue("login"), "password": r.FormValue("password"),
		"name": r.FormValue("name"), "topic_id": r.FormValue("topic_id"),
		"question_text": r.FormValue("question_text"), "correct_answer": r.FormValue("correct_answer"),
		"wrong_answer1": r.FormValue("wrong_answer1"), "wrong_answer2": r.FormValue("wrong_answer2"),
